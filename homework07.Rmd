...
The length that was removed is equal to the sum $2(1/2 + 1/4 + 1/8 + ...)$ (assuming no overlap of intervals), which by the geometric series is equal to $2\frac{1/2}{1-1/2} = 2$. This means that the total length that was not included in the intersection is equal to 1. Therefore $F$ is nonempty. 
\end{solution}
...
\begin{aligned}
|x^2 + x -1 - 5| &< \epsilon \\
-\epsilon < (x^2 + x -6) &< \epsilon \\
-\epsilon < ((x + 1/2)^2 -6 - 1/4) &< \epsilon \\
25/4-\epsilon < ((x + 1/2)^2) &< \epsilon + 25/4 \\
\sqrt{25/4-\epsilon} < (x + 1/2) &< \sqrt{\epsilon + 25/4} \\
\sqrt{25/4-\epsilon} - 5/2 < (x - 2) &< \sqrt{\epsilon + 25/4} - 5/2 \\
-\delta < (x - 2) < \delta
\end{aligned}
$$

Let $\delta = \min\{ (-\sqrt{25/4-\epsilon} + 5/2), (\sqrt{\epsilon + 25/4} - 5/2\})$. 

I will show that $\sqrt{\epsilon + 25/4} - 5/2$ is the larger one.



I will show that $0 < |x-c| < \delta$ implies $|f(x) - L| < \epsilon$.
...
\begin{solution}
Consider the function $f: A \to \mathbb{R}$ where $f(x) = x$ is continous at a point $c \ne 0$, and $c \in \mathbb{R}$

Let $\delta = \epsilon$. Then it follows that
...
Thus we see that $x$ is continuous. Note that $x = \sqrt[3]{x} \cdot \sqrt[3]{x} \cdot \sqrt[3]{x}$ \\

Then by the Algebraic Continuity Theorem (iii) we see that $x$ is a continious at point $c$; therefore, $\sqrt[3]{x}$ must be continous at $c$.  
\end{solution}
